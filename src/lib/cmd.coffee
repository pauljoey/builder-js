...
exports.watch = watchTarget = (target, project, options) ->
	options ?= {watch: true}
	options.watch ?= true
...
exports.runBuilderFile = ->
	...
	project = base.lastProject()
	run(arg, project, options) for arg in options.arguments
...
printTargets = ->
	...
	targets = base.lastProject().getTargets()
